...
export class PaymentService {
  ...
  async simulatePaymentProcessing( amount: number, currency: string): Promise<{ statusCode: number; body: any }> {
    ...
    return {
      statusCode: 200,
      body: { amount, currency, message: 'Payment processed' },
    };
  } 
  ...
  async processPaymentService(ipAddress: string, idempotencyKey: string, body: ProcessPaymentDto): Promise<ProcessRaymentResponseDto> {
    ...
    if (!record) { 
      ...
      // Simulate payment processing delay
      const result = await this.simulatePaymentProcessing(body.amount, body.currency);
      ...
      return {
        ...
        previousResponseData: {
            previousResponseStatusCode: res.statusCode,
            previousResponseBody: res.body, 
         }
    }
  }
}
